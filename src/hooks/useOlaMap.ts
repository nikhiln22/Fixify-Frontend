...
export const useOlaMap = ({ onLocationChange, defaultLocation }: UseOlaMapProps = {}) => {
  ...
  const initializeMap = useCallback((mapLocation: MapLocation) => {
    ...
    try {
      ...
      showToast({
        message: 'Failed to initialize map: ' + (error instanceof Error ? error.message : 'Unknown error'),
        type: 'error',
      });
    }
  }, [onLocationChange]);
  ...
  const getCurrentLocation = useCallback(async () => {
    ...
    try {
      ...
      showToast({
        message: errorMessage + '. Please select a city manually or check your browser settings.',
        type: 'error',
      });
  ...
  const setManualLocation = useCallback((newLocation: MapLocation) => {
    setLocation(newLocation);
    if (mapInstance) {
