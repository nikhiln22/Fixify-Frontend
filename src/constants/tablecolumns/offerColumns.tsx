...
export const getOffersColumns = (
  ...
  },
  {
    ...
    label: "Offer Title",
    render: (item) => (
      <div className="text-center font-medium">{item?.title || "Untitled"}</div>
    ),
  },
  {
    key: "offer_type",
    ...
    render: (item) => (
      <div className="text-center">
        <span className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">
          {item?.offer_type === "global" && "Global"}
          {item?.offer_type === "service_category" && "Service"}
          {item?.offer_type === "first_time_user" && "First Time"}
        </span>
      </div>
    ),
  },
