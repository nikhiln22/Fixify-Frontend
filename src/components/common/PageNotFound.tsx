...
export const PageNotFound: React.FC<PageNotFoundProps> = ({ 
  ...
  return (
    <div className={`min-h-screen ${config.bgColor} flex items-center justify-center px-4`}>
      <div className="max-w-2xl w-full text-center">
          ...
          <ul className="space-y-2 text-left">
            {config.suggestions.map((suggestion, index) => (
              <li key={index} className="flex items-center text-gray-600">
                <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${config.color} mr-3 flex-shrink-0`}></div>
                {suggestion}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-12 text-center">
          <p className="text-gray-500 text-sm">
            ...
            <a href="mailto:support@fixify.com" className={`text-transparent bg-clip-text bg-gradient-to-r ${config.color} hover:underline`}>
              support@fixify.com
            </a>
          </p>
        </div>
